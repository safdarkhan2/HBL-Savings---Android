...
class CustomerInfoUseCaseTest : BaseUTTest() {

    //Target
    private lateinit var customerInfoUseCase: CustomerInfoUseCase

    //Inject customer Info repository created with koin
    val customerInfoRepository by inject<CustomerInfoRepository>()
    ...
    fun start() {
        ...
        customerInfoUseCase = CustomerInfoUseCase()
    }

    @Test
    fun test_personalInfoLookups_use_case_returns_expected_value() = runBlocking {

        mockNetworkResponseWithFileContent("personal_info_lookups.json", HttpURLConnection.HTTP_OK)

        val dataReceived = customerInfoUseCase.getPersonalInfoLookup()

        Assert.assertNotNull(dataReceived)
        Assert.assertEquals(dataReceived.status, statusCode)
//        Assert.as(dataReceived.result.countryList, message)
    }

    @Test
    fun test_save_personalInfo_use_case_returns_expected_value() = runBlocking {
        ...
        val dataReceived = customerInfoUseCase.savePersonalInfo(
    ...
    @Test
    fun test_professionalInfoLookups_use_case_returns_expected_value() = runBlocking {
        ...
        val dataReceived = customerInfoUseCase.getProfessionalInfoLookup()
...
//        Assert.as(dataReceived.result.countryList, message)
    ...
    @Test
    fun test_resume_professional_info_use_case_returns_expected_value() = runBlocking {
        mockNetworkResponseWithFileContent(
            "resume_professional_info.json",
            HttpURLConnection.HTTP_OK
        )

        val dataReceived = customerInfoUseCase.resumeProfessionalInfo(API_ID)
    ...
    @Test
    fun test_save_professionalInfo_use_case_returns_expected_value() = runBlocking {
        ...
        val dataReceived = customerInfoUseCase.saveProfessionalInfo(
    ...
    @Test
    fun test_bankInfoLookups_use_case_returns_expected_value() = runBlocking {

        mockNetworkResponseWithFileContent("bank_info_lookups.json", HttpURLConnection.HTTP_OK)

        val dataReceived = customerInfoUseCase.getBankInfoLookup()
...
//        Assert.as(dataReceived.result.countryList, message)
    ...
    @Test
    fun test_resume_bank_info_use_case_returns_expected_value() = runBlocking {
        mockNetworkResponseWithFileContent("resume_bank_info.json", HttpURLConnection.HTTP_OK)

        val dataReceived = customerInfoUseCase.resumeBankInfo(API_ID)
    ...
    @Test
    fun test_save_bank_account_use_case_returns_expected_value() = runBlocking {
        ...
        val dataReceived = customerInfoUseCase.saveBankInfo(
    ...
    @Test
    fun test_get_bank_by_branch_id_use_case_returns_expected_value() = runBlocking {
        mockNetworkResponseWithFileContent("bank_branches.json", HttpURLConnection.HTTP_OK)

        val dataReceived = customerInfoUseCase.getBranches(16)
    ...
    @Test
    fun test_get_account_title_by_iban_use_case_returns_expected_value() = runBlocking {
        ...
        val dataReceived = customerInfoUseCase.getAccountTitleByIBAN(
